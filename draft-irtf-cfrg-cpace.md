...
guarantees of the simulation-based proof rely on uniqueness of sid {{CPacePaper}}.
...
if CPace is used with unique session-specific values for sid. In this sense, using the wording suggested by Steve Thomas on the CFRG mailing list,
...
It is RECOMMENDED that the ISK is post-processed by a more costly KDF such as {{?RFC5869}}
according the specific needs of the higher-level protocol.

In case that side-channel attacks are to be considered practical for a given application, it is RECOMMENDED to pay
special attention on side-channel protections such as masking and redundant execution (faults) on the process of calculating
